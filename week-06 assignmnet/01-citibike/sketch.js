...
function setup() {
  ...
  const mult = 1.3;
  createCanvas(600*mult, 600*mult);
  background(255);
...
function draw() {
  ...
  const colorR = map(currentTime, trips[0].startedTime, maxTime, 255, 0);
  const colorG = map(currentTime, trips[0].startedTime, maxTime, 0, 255);
  const colorB = 0;
